...
def dct_policy(dag: DAG, target_edges: set, verbose=False, check=False) -> set:
    """
    ...
    while True:
        # Check if target edges already fully oriented. If so, terminate early.
        cpdag_intermediate_check = cpdag.interventional_cpdag(dag, intervened_nodes)
        if len(target_edges.difference(cpdag_intermediate_check.arcs)) == 0:
